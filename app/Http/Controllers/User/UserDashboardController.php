...
class UserDashboardController extends Controller
{
    ...
    public function index()
    {
            ...
            ->profile_id)->with(['transactions' => function ($query)
        {
            ...
            ->get();
        // dd($userDetails);
        return view('user.dashboard', ['userDetails' => $userDetails]);
    }
    ...
    public function viewProfile(Request $request, $id)
    {
        ...
        // dd($id);
        $branch = Branch::all();
        return view('user.create', ['userDetails' => $profile, 'branch' => $branch]);

    }
    public function updateUserDetails(Request $request, $id)
    {
        // dd($request);
        $request->validate(['name' => ['required', 'string', 'max:255'],  'password' => ['required'], 'phone' => ['required', 'digits:10'],'place' => ['required'], 'branch' => ['required'],'img'=>['mimes:jpg,png'] ]);
        $id = Crypt::decrypt($id);
        $profileData=['name' => $request->get('name') ,'place' => $request->get('place'), 'branch_id' => $request->get('branch')];
    ...
    public function qrCodeGenerate()
    {
        $profile = Profile::where('id', auth()->user()
            ->profile_id)
            ->get();
        return view('user.qrcode', ['profile' => $profile]);
    }
    public function qrUserCheck(Request $request)
    {
        ...
        $profile = Profile::where('id', $id)->where('phone', $phoneNo)->
        with('user')->get();
        if (!empty($profile))
        {
            // dd($profile
                ...
                ->user->status == '1')
            {
                ...
                Session::put('id', $id);
                return response()->json(['status' => 1, 'msg' => 'success', 'data' => ['name' => $profile[0]->name, 'url' => '/user/qr-scan/validate'], ]);
            }
            else
            {
                return response()
    ...
    public function finalValidate(Request $request)
    {
        ...
        if (Session::get('readyForPayment'))
        {
            ...
            $to = Profile::where('id', $id)->where('phone', $phoneNo)->with('user')->get();
            $from = Profile::where('id', auth()->user()->profile_id)->
            with('user')->get();
            if (!empty($to))
            {
                if ($to[0]
                    ->user->status == '1')
                {
                    ...
                    ->user->status == '1')
                    {
                    return view('user.payment',['from' => $from,'to' => $to]);
                    }else{
                        auth()->logout();
                ...
                {
                    return response()
                        ->json(['status' => 2, 'msg' => 'User account cannot accept payment', 'url' => '/user/qr-scan/status']);
                }
            }
            else
            {
                return response()
                    ->json(['status' => 0, 'msg' => "Can\'t  find user",  'url' => '/user/qr-scan/status']);
            }
    ...
    public function payAmount(Request $request){
            $userTransactions = Transactions::where('profile_id', auth()->user()->profile_id)->get();
                   $totalAmount=0;
        ...
        $request->validate
         ([
          'amount' => 'required|numeric|max:'.$totalAmount.'',
         ]);
       ...
       if($sendAmount->save()){
        $getAmount->save();
        $request->session()->forget(['token', 'id','readyForPayment']);

        return view('user.payment-success',['from' =>Profile::where('id', auth()->user()
            ->profile_id)->get(),'to' => Profile::where('id', $sendTo)
            ->get() ,'amount' => $amount]);
       }else{
                    return redirect('/user/qr-scan/validate')->with('error-msg','Error occured' );

       }  
        }else{
            return redirect('/user/qr-scan/validate')->with('error-msg','Insufficient Balance' );
            }
        }else{
            return redirect('/user/qr-scan/validate')->with('error-msg','Unauthorized access' );

        }
        }else{
        return redirect('/user/qr-scan/validate')->with('message','Incorrect Pin' );
